...
/*****************************************************************
*
***************App parameters added by jacky*********************/
#define MAX_ENDPOINTKIND 10
#define MAX_NUMBER       10



uint8 AddID;
devStates_t SerialNwkState;
volatile uint16 AddGroup[MAX_ENDPOINTKIND][MAX_NUMBER];
...
#define KEY2_PRESS_EVENT 0x1111

uint8 *time_ok = "time start ok";
uint8 *time_err = "time not start";
void prcess_key2(void);
...
void Timer_led_Init(uint8 task_id)
{
  byte ledTimer;
  Led_TaskID = task_id;
 
  ledTimer = osal_start_timerEx(Led_TaskID, HAL_LED_BLINK_EVENT, 1000);
  if(ledTimer == ZSUCCESS)
  {
    HalUARTWrite(HAL_UART_PORT_0, time_ok, 20);
  }
  else
    HalUARTWrite(HAL_UART_PORT_0, time_err, 20); 
  /*
  ...
  if(bind_cfg_flag)
  {
    osal_start_timerEx(Led_TaskID, HAL_LED_BLINK_EVENT, 100);
  }
...
void SerialApp_Init( uint8 task_id )
{
  ...
  /*******************App Section Added By Jacky***********************/
  SerialNwkState = DEV_INIT;
  
  for(i = 0; i < MAX_ENDPOINTKIND; i++)
  {
  ...
  osal_start_timerEx(SerialApp_TaskID, HAL_SLEEP_TIMER_EVENT, 100);
}


UINT16 Timer_ledProcess(uint8 task_id, UINT16 events)
{
...
#if defined( ZDO_COORDINATOR )
  afSetMatch(SerialApp_epDesc.simpleDesc->EndPoint, TRUE);
