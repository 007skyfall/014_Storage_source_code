...
void ZigBee_IRQ_Process(void)
{
  ...
  if(rd == 0x04)
  {
	...
	UART0_PutChar(SPI752_rbuf_1[SPI752_rbuf_1_ip]);//zigbee to uart
    SPI752_rbuf_1_ip ++;
...
void ZigBee_Send(uint8_t *BufferPtr, uint32_t Length)
{
 
  while ( Length != 0 )
  {
	  SPI752_PutChar(1, *BufferPtr);
      BufferPtr++;
      Length--;
  }
  return;
...
* Return         : None
*******************************************************************************/
void ZigBee_Test(dev_data_t  dev)
{
//  Menu_TypeDef	menu;
  uint8_t	buf[16];
  uint8_t	rd;
