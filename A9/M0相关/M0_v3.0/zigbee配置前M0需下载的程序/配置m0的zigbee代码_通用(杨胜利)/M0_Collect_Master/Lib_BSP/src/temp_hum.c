...
* Return         : None
*******************************************************************************/
void Temp_Hum_Test(dev_data_t *dev)
{
  uint8_t	temp[2], hum[2];
  char		buf[20];
  uint32_t	i	 ;
  static uint32_t fan_on=0;

 //  OLED_ClearScreen();
  OLED_DisStrLine(0, 0, "Temp &Hum &lux &fid");
 // printf("\r\nTemp & Hum\r\n");
