			...
			uint32_t single, uint32_t event )
{
  switch ( portNum )
  {
	case PORT0:
	  if ( sense == 0 )
	  {
		LPC_GPIO0->IS &= ~(0x1<<bitPosi);//边沿触发方式
		/* single or double only applies when sense is 0(edge trigger). */
		if ( single == 0 )
		  LPC_GPIO0->IBE &= ~(0x1<<bitPosi);//中断触发取决于IEV寄存器的状态
		else
		  LPC_GPIO0->IBE |= (0x1<<bitPosi); //both edges trigger
	  }
	  else                           //level trigger
	  	LPC_GPIO0->IS |= (0x1<<bitPosi);
	  if ( event == 0 )                   //falling edge or low level
  ...
  switch ( portNum )
  {
	case PORT0:
	  if ( LPC_GPIO0->MIS & (0x1<<bitPosi) )
		regVal = 1; //interrupt status is 1
	break;
 	case PORT1:
...
void GPIOIntClear( uint32_t portNum, uint32_t bitPosi )
{
  ...
  {
	case PORT0:
	  LPC_GPIO0->IC |= (0x1<<bitPosi);  //clear the interrupt
	break;
 	case PORT1:
...
{
  LPC_GPIO[portNum]->MASKED_ACCESS[(1<<bitPosi)] = (bitVal<<bitPosi);//写相应位
}


uint32_t GPIOGetValue(uint32_t portNum, uint32_t bitPosi)
{
  return LPC_GPIO[portNum]->MASKED_ACCESS[(1<<bitPosi)];     //获得相应位状态
}
...
**
*****************************************************************************/
void GPIOSetDir( uint32_t portNum, uint32_t bitPosi, uint32_t dir )   //设置IO口输入输出状态
{
  if(dir)
