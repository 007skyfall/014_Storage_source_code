...
static volatile uint32_t I2CMasterState = I2C_IDLE;   //0
static volatile uint32_t I2CSlaveState = I2C_IDLE;    //0
...
void I2C_IRQHandler(void) 
{
  ...
  switch ( StatValue )
  {
	case 0x08:			/* A Start condition is sended. */
	WrIndex = 0;
	LPC_I2C->DAT = I2CAddr; //把器件地址写入相应寄存器和W方向
	LPC_I2C->CONCLR = (I2CONCLR_SIC | I2CONCLR_STAC);//清除中断标志和开始标志
	...
	I2CMasterState = I2C_RESTARTED;   //3
	break;
	...
	if ( I2CMasterState == I2C_STARTED )
	{
	  ...
	  I2CMasterState = DATA_ACK;  //可写数据标志 4
	}
	LPC_I2C->CONCLR = I2CONCLR_SIC; //清中断
	break;
	
	case 0x28:	/* Data byte has been transmitted, regardless ACK or NACK */
	case 0x30:
	if ( WrIndex < I2CWriteLength )  //没有写完继续写
	{   
	  LPC_I2C->DAT = I2CMasterBuffer[WrIndex++]; /* this should be the last one */
	  I2CMasterState = DATA_ACK;  //应答标志 4
	}
	else
	{
	  if ( I2CReadLength != 0 )
	  {
		...
		I2CMasterState = I2C_REPEATED_START;    //3 继续向
	  }
	  else
	  {
		I2CMasterState = DATA_NACK;   //无应答标志 5
	...
	case 0x58:
	I2CSlaveBuffer[RdIndex++] = LPC_I2C->DAT;    //读出数据寄存器的数据
	I2CMasterState = DATA_NACK;                  //设置读标志
	LPC_I2C->CONSET = I2CONSET_STO;	/* Set Stop flag */ 
	...
	default:
	LPC_I2C->CONCLR = I2CONCLR_SIC;	   //数据丢失，清中断
	break;
  }
  return;
}
...
uint32_t I2CStart( void )
{
  ...
  while( 1 )
  {
	if ( I2CMasterState == I2C_STARTED )      //1,可以开始传输数据
	{
	  retVal = TRUE;
	  break;	
	}
	if ( timeout >= I2C_MAX_TIMEOUT )        //0x00FFFFFF
	{
	  retVal = FALSE;
	  break;
	}
...
uint32_t I2CInit( uint32_t I2cMode, uint32_t slaveAddr )
{
  ...
  /*--- Reset registers ---*/
#if FAST_MODE_PLUS            //FAST_MODE_PLUS=0
  LPC_IOCON->PIO0_4 |= (0x1<<9);
  LPC_IOCON->PIO0_5 |= (0x1<<9);
  LPC_I2C->SCLL   = I2SCLL_HS_SCLL;  
  LPC_I2C->SCLH   = I2SCLH_HS_SCLH;
#else
  LPC_I2C->SCLL   = I2SCLL_SCLL;   //低电平计数
  LPC_I2C->SCLH   = I2SCLH_SCLH;   //高电平计数
#endif
...
uint32_t I2CEngine( void )
{
  ...
  while ( 1 )
  {
	if ( I2CMasterState == DATA_NACK )    //DATA_NACK=5 在中断中设置
	{
	  I2CStop();
	  break;
	}
...
void I2CRead(uint8_t addr, uint8_t *buf, uint32_t len)
{
    ...
    I2CReadLength = I2CReadLength;     //在中断中改变
}
...
void I2CWrite(uint8_t addr, uint8_t* buf, uint32_t len)
{
    ...
    I2CWriteLength = I2CWriteLength;    //在中断中改变
}
