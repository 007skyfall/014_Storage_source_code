...
void Main_PLL_Setup ( void )
{
  ...
  regVal = LPC_SYSCON->SYSPLLCTRL;                 //读出系统倍频寄存器的数据
  regVal &= ~0x1FF;                                //清除低9位
  LPC_SYSCON->SYSPLLCTRL = (regVal | (MAIN_PLL_P_Val<<5) | MAIN_PLL_M_Val);//设置时钟频率为96MHz
...
#if MAIN_PLL_SETUP
  SystemFrequency = ClockSource * (MAIN_PLL_M_Val+1);     //48M
#else
  SystemFrequency = ClockSource;    //12M
#endif
