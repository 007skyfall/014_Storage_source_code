...
void delay32Ms(uint8_t timer_num, uint32_t delayInMs)
{
  ...
  else if (timer_num == 1)
  {
    ...
    /* wait until delay time has elapsed */
    while (LPC_TMR32B1->TCR & 0x01); //当MR0值与TC值相等时，CRst为1
  }
  return;
}
...
void TIMER32_0_IRQHandler(void)
{
  ...
  {  
	...
	timer32_0_counter++;
    if (timer32_0_counter>=1000)
    timer32_0_counter=0;
	if(timer32_0_counter < pwm)			//改变风扇转数
	{
...
void reset_timer32(uint8_t timer_num)
{
  ...
  {
    regVal = LPC_TMR32B1->TCR;    //不破坏原来数据
    regVal |= 0x02;               //置位CRst
...
void init_timer32(uint8_t timer_num, uint32_t TimerInterval) 
{
  if ( timer_num == 0 )
  {
    ...
    LPC_SYSCON->SYSAHBCLKCTRL |= (1<<9); //使能定时器时钟
    LPC_IOCON->PIO1_5 &= ~0x07;	/*  Timer0_32 I/O config */
    LPC_IOCON->PIO1_5 |= 0x00;	/* Timer0_32 CAP0 */
    ...
    LPC_IOCON->R_PIO0_11 &= ~0x07;	 //模数转换引脚
...
#ifdef __JTAG_DISABLED              //timer32的引脚和JTAG的引脚复用
...
void init_timer32PWM(uint8_t timer_num, uint32_t period, uint8_t match_enable)
{
  ...
  if (timer_num == 1)
  {
    ...
    LPC_TMR32B1->MR3 = timer32_1_period;     //之所以有4个寄存器，
