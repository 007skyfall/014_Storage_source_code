...
uint8_t RFID_Operate_test(uint8_t *tbuf, uint8_t *rbuf)
{
  uint8_t	chksum;
  uint32_t	i, j, rnumb;
...
/*******************************************************************************
* Function Name  : RFID_Menu_01
* Description    : RFID test Menu.
* Input          : - numb, Function number
* Output         : None
* Return         : None
*******************************************************************************/
void RFID_Menu_01(uint32_t numb)
{
  OLED_ClearScreen();
  ...
  if((numb>=1) && (numb<=6))
  {
    ...
    if(numb == 1)
      OLED_DisStrLine(2-1 + 1, 1, "1.Read Card");
    else
  	  OLED_DisStrLine(2-1 + 1, 0, "1.Read Card");
  
    if(numb == 2)
      OLED_DisStrLine(2-1 + 2, 1, "2.Write Card");
    else
  	  OLED_DisStrLine(2-1 + 2, 0, "2.Write Card");
  }
