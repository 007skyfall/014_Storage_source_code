...
void RFID_Test(void)
{
  ...
  menu.item[1] = "2.Write data1";
  menu.item[2] = "3.Write data2";
		...
		case 2:
			if(!GPIOGetValue(PORT2, 8))
			{
				...
				j = RFID_Operate((uint8_t *)RFID_WRITE_DATA_BLOCK_22_0, rbuf);	  
	          	OLED_DisStrLine(2-1 + 6, 0, "write 0123456789 Ok");
              	printf("\r\nOk ");
	        }
            else
	        {
		...
		case 3:
		   if(!GPIOGetValue(PORT2, 8))
		   {
			...
			j = RFID_Operate((uint8_t *)RFID_WRITE_DATA_BLOCK_22_1, rbuf);	
	         OLED_DisStrLine(2-1 + 6, 0, "write ABCDEFGHIJ  Ok");
             printf("\r\nOk ");
	        }
		else
			{
