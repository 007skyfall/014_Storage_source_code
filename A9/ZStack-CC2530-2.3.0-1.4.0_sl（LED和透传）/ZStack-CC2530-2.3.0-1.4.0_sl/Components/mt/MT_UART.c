...
//  App_TaskID = 0;
  ...
  uartConfig.baudRate             = HAL_UART_BR_115200;//MT_UART_DEFAULT_BAUDRATE;
...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  uint8 buf[128]; //串口buffer最大缓冲默认是128，我们这里用128.  
    ...
    if (event & /*( HAL_UART_RX_FULL | HAL_UART_RX_ABOUT_FULL |*/ HAL_UART_RX_TIMEOUT)//)
    {
      ...
      if(RxLen > 0)
      {
        HalUARTRead (port, buf , RxLen);
//        HalUARTWrite(port,buf, RxLen);
#ifdef GENERIC_SERIALAPP        
        SampleApp_SendMessage(buf,RxLen);
#endif
#ifdef SERIALAPP
        SampleApp_SendMessage(buf,RxLen);
#endif
