...
{
	unsigned char goods_type;//               物品类型
	unsigned int goods_count; //              物品数量
};
...
struct storage_info
{
	...
	float humidityMIN;
	float humidityMAX;
	float illumination;    //                仓库当前光照
	float illuminationMIN;
	float illuminationMAX;
	float battery;           //                仓库采集端电池电压
	float adc;                 //             仓库ADC采集电压
	float adcMIN;                //             仓库电池电压最小值
	struct storage_goods_info goods_info[GOODS_NUM]; //       采集货物信息
};

struct env_info_clien_addr
{
	struct storage_info storage_no[STORAGE_NUM];//	 仓库实时信息
};
...
struct msg
{
	...
	long msgtype; //            区别消息的类型
	unsigned char text[QUEUE_MSG_LEN];//       消息正文长度
};
